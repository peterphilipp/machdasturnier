...
export interface Volunteer { id: number; name: string; email: string | null; phone: string | null; role?: 'HELPER' | 'ORGANIZER' | 'ADMIN' | 'TRAINER'; roles?: ('HELPER' | 'ORGANIZER' | 'ADMIN' | 'TRAINER')[]; tournamentId: number | null; consentGiven?: boolean; consentDate?: string; createdAt?: string; lastLoginAt?: string | null; lastActivityAt?: string | null; children?: { childName: string; childYear: number }[]; pushSubscriptions?: { id: number; userAgent?: string | null; createdAt?: string; deviceLabel?: string }[]; trainedYearGroups?: { id: number; name: string }[]; /** Helfer ohne App-Zugang - kein Konto, keine Benachrichtigungen an ihn selbst. */ ohneZugang?: boolean; kontaktpersonId?: number | null; kontaktperson?: { id: number; name: string } | null; /** Zusagen/Absagen/Verschiebungen zusaetzlich per Mail - Default an. */ mailBenachrichtigungen?: boolean; }
